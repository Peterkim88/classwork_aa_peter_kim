...
def all_vowel_pairs(words)
    ...
    (0...words.length-1).each do |idx1|
        (idx1+1...words.length).each do |idx2|
            word_pairs << "#{words[idx1]} #{words[idx2]}"
        end
    end
    word_pairs.select { |pair| vowels.all? { |vowel| pair.include?(vowel) } }
end
...
class String
    ...
    def substrings(length = nil)
        subs = []
        (0...self.length).each { |idx1| (idx1...self.length).each { |idx2| subs << self[idx1..idx2] } }
